...
Also, I have an image below and save it as `og-background.png` in the `public` directory. This will be the background image for the OG image.

![Background image for OG image with color gradient and noise texture](/og-background.png)
...
### Convert HTML to OG image

The last step is very simple, we just need to convert the SVG to PNG image.
Unfortunately, the Satori requires the font buffers. There are two ways to do this:
